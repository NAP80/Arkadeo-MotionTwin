...
// Une pièce qui court de start à end le long du labyrinthe puis s'y pose (end.addCoin) - utilisé par seq.SpitCoins.
class RunningCoin extends mt.fx.Fx {
	...
	public function new(start, end) {
		...
		// buildDistFrom(end) ci-dessus a écrasé la carte de distance PARTAGÉE
		// (squares[].hdist) que l'IA des monstres utilise pour viser le héros.
		// Le chemin est calculé : on restaure la carte centrée sur le héros, sinon
		// (héros immobile, pas de majHeroDist) les monstres foncent vers la pièce.
