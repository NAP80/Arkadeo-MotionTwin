...
// Héros : déplacement au clavier (KeyboardManager), ramassage des pièces,
// Bonus saut (saute un mur) / étoile (invincibilité).
class Hero extends Ent {
	...
	public function applyBonus(kind:BonusKind) {
		bonus = kind;
		bonusLife = Cs.BONUS_LIFE;
		// Nouvelle casquette -> le combo de kills repart de zéro.
		if (kind == BonusKind.BK_Star) Game.me.starCombo = 0;
		updateSkin();
	}
	...
	override function update() {
		...
		inputX = -1;
		inputY = -1;
		if (api.AKApi.isDown(KeyboardManager.RIGHT)) inputX = 0;
		if (api.AKApi.isDown(KeyboardManager.DOWN)) inputY = 1;
		if (api.AKApi.isDown(KeyboardManager.LEFT)) inputX = 2;
		if (api.AKApi.isDown(KeyboardManager.UP)) inputY = 3;
		if (api.AKApi.isToggled(KeyboardManager.SPACE)) {
