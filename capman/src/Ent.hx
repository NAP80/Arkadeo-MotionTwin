...
// Entité posée sur la grille : déplacement case-à-case interpolé (moveCoef/EStep).
class Ent {
	...
	public function gotoSquareId(id) {
		...
		if (sq == null) return; // id hors grille (données héritées) -> on ignore
		setSquare(sq.x, sq.y);
	}

	public function setSquare(x, y) {
		// Garde anti-hors-grille : un monstre peut viser une case null -> on refuse le déplacement, sinon crash sur square.x plus loin.
