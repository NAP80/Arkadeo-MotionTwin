...
// initTarget cherche une cible à 5-10 cases par tirages successifs. Si le Fly est hors
// map (poussé par une rafale de charges), aucune cible de la grille ne convient et la
// boucle ne se termine jamais : d'où le garde-fou à 50 essais max.
class Fly extends Mob {
	...
	function initTarget() {
		...
		// Garde-fou anti-boucle : après 50 essais, on accepte le dernier tirage.
		var tries = 0;
		while ( (d2<5*5 || d2>10*10) && tries++ < 50 ) {
