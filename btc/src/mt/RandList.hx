...
// Tirage pondéré. fromMap est hors #if flash (la lib d'origine le gatait Flash, mais
// LevelGenerator/Level l'utilisent côté JS).
class RandList<T> {
	...
	public function toString() {
		var list = [];
		for (e in drawList)
			list.push(Std.string(e.value) + " => " + Math.round(1000 * e.proba / totalProba) / 10 + "%");
		return list.join(" | ");
	}
}
