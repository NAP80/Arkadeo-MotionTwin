...
// Piège : registerStateAnim démarre la boucle d'update (startUpdates). Sans ça,
// une entité qui n'enregistre que des state anims sans play() initial ne tourne
// jamais → sprite invisible jusqu'au 1er play().
...
class AnimManager {
	...
	inline function getCurrentAnim() return animStack[0];
	inline function getLastAnim() return animStack[animStack.length - 1];
	inline function startUpdates() needUpdates = true;
	inline function stopUpdates() needUpdates = false;
	...
	inline function hasAnim() return !destroyed && animStack.length > 0;
	...
	public inline function chainAndLoop(k:String) return chain(k).loop();
	public inline function playAndLoop(k:String) return play(k).loop();
	...
	public inline function playAndWait(k:String, durationFrame:Int) {
		return play(k, 1).onEnd(function() suspendFor(durationFrame, k, spr.frame));
	}
	...
	public function play(group:String, ?plays = 1, ?queueAnim = false) {
		...
		if (g.anim == null || g.anim.length == 0) {
			// Groupe sans anim = pose statique (idle/hover/raise…) : afficher sa
			// frame 0 au lieu de garder l'anim précédente (sinon gel = "pause").
		...
		if (!queueAnim && hasAnim())
			stopWithoutStateAnims();
		var a:AnimInstance = {
			group: group, paused: false, curFrameCpt: 0, animCursor: 0, plays: plays,
			frames: g.anim, isStateAnim: false, killAfterPlay: false, onEnd: null, speed: 1.0,
		};
		isPlaying = true;
		animStack.push(a);
		startUpdates();
	...
	public function pause() if (hasAnim()) getCurrentAnim().paused = true;
	public function resume() if (hasAnim()) getCurrentAnim().paused = false;
	...
	inline function isSuspended() return suspendTime > 0;
	...
	public function registerStateAnim(group:String, priority:Int, ?condition:Void->Bool) {
		...
		stateAnims.push({group: group, priority: priority, condition: condition});
		stateAnims.sort(function(a, b) return -Reflect.compare(a.priority, b.priority));
		startUpdates(); // démarre la boucle : les state anims s'appliquent sans play() initial
	}
	...
	public function applyStateAnims() {
		...
		for (sa in stateAnims)
			if (sa.condition()) {
				// déjà sur cette state anim (animée) OU déjà sur cette pose statique
				if ((hasAnim() && getCurrentAnim().group == sa.group) || (!hasAnim() && spr.groupName == sa.group))
					break;
	...
	public function toString() {
		return "AnimManager(" + spr + ")" + (hasAnim() ? "Playing(stack=" + animStack.length + ")" : "NoAnim");
	}
	...
	function _update() {
		if (isSuspended()) {
			...
			if (isSuspended())
				...
				if (hasAnim()) {
					var a = getCurrentAnim();
					spr.set(a.group, a.frames[a.animCursor]);
				}
			}
		}
		...
		// Boucle maintenue tant qu'il reste des state anims (pour réagir aux changements
		// de condition idle/run/fall…), sinon arrêtée.
		if (!destroyed && !hasAnim() && !isSuspended() && (stateAnims == null || stateAnims.length == 0))
			stopUpdates();
	}
}
